...
创建时能显示创建时间，后续修改时会显示修改时的时间。
第一步，在noteslist_item.xml 中添加时间戳的布局，并整体改为LinearLayout布局。
...
第二步，在NotesList.java中的onCreate方法中，给dataColumns和viewIDs增加数据条目：
String[] dataColumns = {
...
第三步，在SimpleCursorAdapter中，增加生成时间，显示时间的方法：
            @Override
...
第二步，在NotesList类的onOptionItemSelected方法中的Switch分支语句中，增加选项菜单search的case：
case R.id.menu_search:
            startActivityForResult(new Intent(this, Search.class), REQUEST_SEARCH);
            return true;

第三步，创建Search类，实现返回查询内容：
...
第四步，在AndroidManifest中注册Search的activity：
...
第五步，在NotesList类中添加方法，完成查询，筛选，排序，显示等逻辑：
    @Override
...
-------------------------------------------------------------我是分界线---------------------------------------------------------------------------------
3.根据笔记标题或修改时间排序（升序）
由于每次修改，最新修改的笔记都会自动到最上方，故时间排序功能把修改时间早的笔记前置。
第一步，在list_options_menu.xml中，添加排序选项的item：
...
第二步，在NotesList类的onOptionItemSelected方法中的Switch分支语句中，增加选项的case：
...
第三步，在NotesList.java中添加方法，实现排序功能的逻辑：
// 按修改时间排序
...
-------------------------------------------------------------我是分界线---------------------------------------------------------------------------------
4.更换主界面的颜色
第一步，在list_options_menu.xml中，添加换背景按钮的item：
<item android:id="@+id/menu_color"
        android:icon="@drawable/ic_menu_color"
        android:title="@string/menu_color"
        android:showAsAction="always" />

第二步，在NotesList类的onOptionItemSelected方法中的Switch分支语句中，增加选项按钮的case：
case R.id.menu_color:
        // 显示颜色选择对话框
        changeBackgroundColor();
        return true;

第三步，在NotesList.java中添加方法，实现换背景功能的逻辑：
